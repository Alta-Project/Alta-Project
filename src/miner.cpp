...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ALTAMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ALTAMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ALTAMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("alta-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("ALTAMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running ALTAMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadALTAMiner() exception");
    } catch (...) {
        LogPrintf("ThreadALTAMiner() exception");
    }

    LogPrintf("ThreadALTAMiner exiting\n");
}
