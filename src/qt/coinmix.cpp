...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::ALTA)
    {
    }
	...
	*/
    ...
    if (fLiteMode) {
        ...
        if (fMasterNode) {
            ...
            if (!fEnableDarksend) {
                ui->toggleDarksend->setText(tr("Start StealthSend"));
            } else {
                ui->toggleDarksend->setText(tr("Stop StealthSend"));
            }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("ALTA")
    //updateDisplayUnit();
}
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeALTAAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeALTAAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeALTAAmount = strAnonymizeALTAAmount.remove(strAnonymizeALTAAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeALTAAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeALTAAmount * COIN) nMaxToAnonymize = nAnonymizeALTAAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeALTAAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeALTAAmount));
        strAnonymizeALTAAmount = strAnonymizeALTAAmount.remove(strAnonymizeALTAAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeALTAAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void Coinmix::toggleDarksend()
{
    ...
    if (hasMixed.isEmpty()) {
        QMessageBox::information(this, tr("StealthSend"),
            tr("If you don't want to see internal StealthSend fees/transactions select \"Most Common\" as Type on the \"Transactions\" tab."),
            QMessageBox::Ok, QMessageBox::Ok);
    ...
    if (!fEnableDarksend) {
        ...
        if (balance < minAmount) {
            ...
            QMessageBox::warning(this, tr("Darksend"),
                tr("StealthSend requires at least %1 to use.").arg(strMinAmount),
                QMessageBox::Ok, QMessageBox::Ok);
            return;
        }
        ...
        if (walletModel->getEncryptionStatus() == WalletModel::Locked) {
            ...
            if (!ctx.isValid()) {
                ...
                QMessageBox::warning(this, tr("Darksend"),
                    tr("Wallet is locked and user declined to unlock. Disabling StealthSend."),
                    QMessageBox::Ok, QMessageBox::Ok);
                if (fDebug) LogPrintf("Wallet is locked and user declined to unlock. Disabling StealthSend.\n");
                return;
            }
        }
    }
